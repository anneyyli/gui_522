...
const statusConfig: Record<string, { bg: string; text: string; label: string }> = {
  IN_OFFICE: { bg: "bg-emerald-100", text: "text-emerald-700", label: "Office" },
  REMOTE: { bg: "bg-blue-100", text: "text-blue-700", label: "Remote" },
  OUT_OF_OFFICE: { bg: "bg-rose-100", text: "text-rose-700", label: "Out" },
          ...
          {rows.map((row) => (
            <tr key={row.employeeId || row.name} className="group">
              ...
              {row.days.map((status, i) => {
                const config = statusConfig[status] || { bg: "bg-slate-100", text: "text-slate-500", label: "—" };
                return (
                  <td key={i} className="py-3 text-center">
                    <span className={`inline-flex min-w-[3.5rem] items-center justify-center rounded-md px-2 py-1.5 text-[11px] font-semibold ${config.bg} ${config.text}`}>
                      {config.label}
                    </span>
                  </td>
                );
